...
t<-cardiacsurgery_tsb %>%
    aggregate_key(Der_Provider_Code, Total = sum(Admissions)) %>%
 mutate (., ds = Month, y = Total)

t_tr <- t %>%
  stretch_tsibble(.init = 36, .step = 6)

fit <- t_tr %>%
  model(base = prophet(log(y)~season("year")+holiday(Lockdowns))) %>%
  reconcile(
    ols = min_trace(base, method = "ols"))%>% 
  forecast(h = "1 year")%>%
  accuracy(t, by = c(".model")) 
...
fc %>%
  autoplot() +
  facet_wrap(vars(Der_Provider_Code), scales = "free_y")
